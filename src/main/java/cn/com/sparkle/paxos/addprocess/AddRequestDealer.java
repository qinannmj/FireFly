...
public class AddRequestDealer implements InstanceExecuteEventListener, InstancePaxosEventListener, MasterChangePosEventListener, SpeedControlEventListener {
				...
				try {
					...
					if (isMaster) {
						if (instanceId == -2) {
							...
							AddRequest nullRequest = new AddRequest(-1, CommandType.ADMIN_WRITE, NULL_BYTES);
	...
	public void add(PaxosSession session, AddRequest addRequest) {
		try {
			...
			if (dealState.isDealing) {
				...
				if (lastPackage != null && lastPackage.getValueByteSize() < curTcpPackageByteSize && !addRequest.isAdmin() && !lastPackage.isManageCommand()) {
	...
	@Override
	public void instanceExecuted(cn.com.sparkle.paxos.stablestorage.model.StoreModel.SuccessfulRecord record) {
